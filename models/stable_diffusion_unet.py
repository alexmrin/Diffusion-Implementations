...
class Unet(nn.Module):
    def __init__(
        ...
        for i in range(levels):
            for _ in range(n_res_blocks):
                ...
                if i in attention_levels:
                    layers.append(SpatialTransformer(channels, tf_layers, d_cond, channels_per_head))
        ...
        self.middle_block = SwitchSequential(
            ...
            SpatialTransformer(channels, tf_layers, d_cond, channels_per_head),
            ResBlock(channels, d_time_emb)
        )
...
def _test_time_embeddings():
    ...
    m = Unet(in_channels=1, out_channels=1, channels=320, n_res_blocks=1, attention_levels=[],
                  channel_multipliers=[],
                  channels_per_head=1, tf_layers=1, d_cond=1, t_max=1000)
...
def _test_unet():
    m = Unet(in_channels=1, out_channels=1, channels=64, n_res_blocks=1, attention_levels=[1],
                  ...
                  channels_per_head=1, tf_layers=1, d_cond=256, t_max=1000)
    time = torch.randint(0, 1000, (10,))
    cond = torch.randn((10, 5, 256))
    input = torch.randn(10, 1, 32, 32)
    output = m(input, time, cond)
    print(output.shape)
